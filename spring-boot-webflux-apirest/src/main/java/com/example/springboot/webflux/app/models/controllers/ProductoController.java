package com.example.springboot.webflux.app.models.controllers;
...
import com.example.springboot.webflux.app.SpringBootWebfluxApirestApplication;
...
public class ProductoController {
    ...
    private final SpringBootWebfluxApirestApplication springBootWebfluxApirestApplication;
    ...
    ProductoController(SpringBootWebfluxApirestApplication springBootWebfluxApirestApplication) {
        this.springBootWebfluxApirestApplication = springBootWebfluxApirestApplication;
    }
	
	@PostMapping("/data/con-foto")
	public Mono<ResponseEntity<Producto>> crearConFoto( Producto producto, @RequestPart FilePart file){
		...
		return file.transferTo(new File(ruta_imagen+producto.getFoto())).then(service.save(producto))
		.map(p -> {
	...
	public Mono<ResponseEntity<Producto>> upload(@PathVariable String id, @RequestPart FilePart file){
		...
		return service.findById(id).flatMap(p -> {
			...
			return file.transferTo(new File(ruta_imagen+p.getFoto())).then(service.save(p));
		...
		return monoProducto.flatMap(producto -> {
			...
			return service.save(producto).map(p -> {
				...
				return Mono.just(t).cast(WebExchangeBindException.class)
						...
						.map(fieldError -> "El campo "+fieldError+" "+fieldError.getDefaultMessage())
						.collectList()
						.flatMap(list -> {
